...
### Patterns
Patterns are downloaded with the script `download-patterns.sh` which come as 1896 .rle (run-length encoded) files. The full list can be found [here](http://www.conwaylife.com/wiki/Category:Patterns) This must be done before running. My program includes a decoder that can parse this format and use it to seed the grid.
